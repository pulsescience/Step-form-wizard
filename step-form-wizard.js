...
(function($) {
	...
	stepFormWizard.prototype.setProportion = function(animation) {
		...
		if(w.config.height == 'auto' && w.config.transition != '3d-cube' && w.steps.length) {
			w.updateHeight(); // Используем новый метод
			return; // Ранний выход, так как высота уже обновлена
		}
	...
	stepFormWizard.prototype.afterTransition = function(to, index, data) {
		...
		// Обновляем высоту после перехода
		if(w.config.height == 'auto') {
			w.updateHeight();
		}
	...
	stepFormWizard.prototype.updateHeight = function() {
		var w = this;
		if(w.config.height == 'auto') {
			var currentStep = w.step(w.stepActive);
			currentStep.height('auto');
			var newHeight = currentStep.outerHeight(true);
			w.viewPort.height(newHeight);
			
			// Для других переходов кроме 3D-cube
			if(w.config.transition != '3d-cube') {
				$('.sf-step', w.wizard).each(function() {
